...
try:
    ...
    print("\n[!] Missing packages. Please install them by running:")
    print("    pip install flask opencv-python numpy matplotlib\n")
    sys.exit(1)
...
<head>
  ...
  <title>Gargi's executed IP practicals | S. B. Jain Institute (N-PECCS502P)</title>
...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans flex flex-col antialiased">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span class="text-[10px] bg-amber-500/20 text-amber-400 font-semibold px-1.5 py-0.5 rounded border border-amber-500/30">NAAC 'A'</span>
            </div>
            <p class="text-[11px] text-slate-400">Gargi Makhe • USN: <strong class="text-slate-300 font-mono">CS24204</strong> • Image Processing Lab (N-PECCS502P)</p>
          </div>
        </div>
      </div>
    ...
    <main class="lg:col-span-8 space-y-5">
      <div class="bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-indigo-900/40 rounded-2xl p-5 shadow-lg">
        <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
          ...
          <div class="flex gap-2 text-xs">
            <button onclick="switchTab('sim')" id="tabBtnSim" class="px-3 py-1 rounded bg-indigo-600 text-white font-medium">Interactive Simulator</button>
            <button onclick="switchTab('theory')" id="tabBtnTheory" class="px-3 py-1 rounded bg-slate-800 text-slate-300 font-medium hover:text-white">Working Principle & Theory</button>
            <button onclick="switchTab('code')" id="tabBtnCode" class="px-3 py-1 rounded bg-slate-800 text-slate-300 font-medium hover:text-white">Python Script</button>
          </div>
        </div>
        <h2 id="pTitle" class="text-xl font-black text-white">Introduction & Environment Setup</h2>
      ...
      <div id="viewSim" class="space-y-5">
        <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div class="flex items-center gap-2">
            ...
            <select id="preset" onchange="loadPreset(this.value)" class="bg-slate-800 border border-slate-700 rounded px-2.5 py-1 text-slate-200">
              <option value="portrait">Lenna (Color Portrait)</option>
              <option value="cameraman">Cameraman (Contrast Grayscale)</option>
              <option value="shapes">Geometric Shapes (Binary / Morphology)</option>
              <option value="damaged">Damaged Photo (For Inpainting)</option>
            </select>
          </div>
          <div class="flex items-center gap-2">
            <label for="imgFile" class="cursor-pointer bg-slate-800 hover:bg-slate-700 text-indigo-300 border border-indigo-500/40 px-3 py-1 rounded flex items-center gap-1.5 transition">
              <i class="fa-solid fa-upload"></i> Upload Custom Image
            </label>
            <input type="file" id="imgFile" accept="image/*" class="hidden" onchange="uploadCustom(event)" />
            <button onclick="resetImg()" class="bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1 rounded border border-slate-700" title="Reset Image">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col items-center">
            <div class="w-full flex justify-between text-xs text-slate-400 mb-2">
              <span class="font-semibold uppercase tracking-wider">Inserted Input</span>
              <span class="font-mono text-[11px]">256 x 256 px</span>
            </div>
            <div class="bg-black/50 border border-slate-800 rounded-xl p-1 w-full h-[260px] flex items-center justify-center">
              <canvas id="cOrig" width="256" height="256" class="max-w-full max-h-full object-contain"></canvas>
            </div>
          </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <div class="flex justify-between text-xs text-slate-400 mb-2">
            ...
            <span class="text-[10px]">0 (Dark) to 255 (Bright)</span>
          </div>
      ...
      <div id="viewCode" class="hidden bg-slate-900 border border-slate-800 rounded-2xl p-5">
        <div class="flex justify-between items-center mb-3">
          ...
          <button onclick="copyCode()" class="text-xs bg-indigo-600 hover:bg-indigo-500 px-3 py-1 rounded text-white flex items-center gap-1">
            <i class="fa-solid fa-copy"></i> Copy Code
          </button>
        </div>
        <pre class="bg-slate-950 p-4 rounded-xl text-xs text-emerald-400 overflow-x-auto"><code id="codeText"></code></pre>
      </div>
  ...
  <div id="modalReport" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-700 max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 md:p-8 rounded-2xl space-y-5 text-xs text-slate-300">
      <div class="flex justify-between items-center border-b border-slate-800 pb-3">
        <div>
          <span class="text-[10px] uppercase font-bold text-amber-400 tracking-wider">Autonomous Institution Affiliated to RTMNU</span>
          <h3 class="text-base md:text-lg font-extrabold text-white">S. B. JAIN INSTITUTE OF TECHNOLOGY, MANAGEMENT & RESEARCH, NAGPUR</h3>
          <p class="text-[11px] text-slate-400">Post Lab Report: Image Processing Lab (N-PECCS502P) • V Sem CSE (2026-27)</p>
        </div>
      ...
      <div class="bg-slate-950 p-3.5 rounded-lg border border-slate-800 grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div><span class="text-slate-500 block">Subject Code:</span> <strong class="text-slate-200">N-PECCS502P</strong></div>
      ...
      <div class="space-y-2 bg-slate-950 p-4 rounded-xl border border-slate-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px] leading-relaxed">
          <div>
            <strong class="text-slate-200 block mb-0.5">• Front-End:</strong>
            HTML5 Canvas 2D API, Modern JavaScript (ES6+), CSS3 & Tailwind CSS (v3.4), Chart.js (Intensity Histograms), FontAwesome Icons.
          </div>
          <div>
            <strong class="text-slate-200 block mb-0.5">• Back-End & Execution Engine:</strong>
            Python 3.10+, Flask (v3.0) Micro-framework, Multithreading asynchronous execution.
          </div>
          <div>
            <strong class="text-slate-200 block mb-0.5">• APIs & Communication Protocol:</strong>
            RESTful API (`/api/process` POST route), JSON communication payload, Base64 lossless image streaming.
          </div>
          <div>
            <strong class="text-slate-200 block mb-0.5">• Computer Vision Libraries:</strong>
            OpenCV (`cv2` v4.8+), NumPy (v1.24+ matrix processing), Matplotlib (v3.7+ spectral plotting).
          </div>
          <div>
            <strong class="text-slate-200 block mb-0.5">• Database & Storage:</strong>
            Local File System Storage (`/outputs` directory for output image matrices), In-memory Base64 session caching.
          </div>
          <div>
            <strong class="text-slate-200 block mb-0.5">• IDE & Deployment:</strong>
            Visual Studio Code (VS Code), Git, GitHub Pages, Localhost server (`http://127.0.0.1:5000`).
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end pt-3 border-t border-slate-800">
        <button onclick="window.print()" class="bg-indigo-600 text-white px-4 py-1.5 rounded font-semibold mr-2 hover:bg-indigo-500">
          <i class="fa-solid fa-print mr-1"></i> Print Report
        </button>
        <button onclick="closeModal()" class="bg-slate-800 text-slate-300 px-4 py-1.5 rounded hover:bg-slate-700">Close</button>
      </div>
    </div>
  </div>
  ...
  <script>
    const PRACTICALS = [
      {
        id: 1, code: 'PRAC-01', title: 'Prelab: Environment Setup & Python Tools',
      ...
      {
        id: 4, code: 'PRAC-04', title: 'Spatial Enhancement & Histogram Equalization',
      ...
      {
        id: 5, code: 'PRAC-05', title: 'Spatial Domain Filters (Smoothing & Noise Removal)',
    ...
    function buildMenu() {
      document.getElementById('sidebarList').innerHTML = PRACTICALS.map(p => `
        <button onclick="selectP(${p.id})" id="btn-${p.id}" class="w-full text-left p-2 rounded-xl transition flex items-start gap-2 hover:bg-slate-800">
          <span class="text-[10px] font-mono font-bold bg-slate-800 text-indigo-400 px-1.5 py-0.5 rounded">${p.code}</span>
          <span class="text-xs font-semibold text-slate-200 truncate">${p.title}</span>
        </button>
      `).join('');
    }
    ...
    function selectP(id) {
      ...
      PRACTICALS.forEach(x => {
        document.getElementById(`btn-${x.id}`).className = x.id === id ? 
          'w-full text-left p-2 rounded-xl bg-indigo-950/50 border border-indigo-500/40 flex items-start gap-2' : 
          'w-full text-left p-2 rounded-xl hover:bg-slate-800 flex items-start gap-2';
      });
      document.getElementById('pBadge').innerText = p.code;
      document.getElementById('pTitle').innerText = p.title;
      document.getElementById('pAim').innerText = p.aim;
      document.getElementById('dynControls').innerHTML = p.ctrl;
      document.getElementById('codeName').innerText = `${p.code.toLowerCase().replace('-', '_')}.py`;
    ...
    function loadPreset(k) {
      ...
      if (k === 'portrait') {
        const g = tc.createRadialGradient(128,128,10,128,128,120);
        g.addColorStop(0, '#fde047'); g.addColorStop(0.5, '#f97316'); g.addColorStop(1, '#3b82f6');
        tc.fillStyle = g; tc.fillRect(0,0,256,256);
        tc.fillStyle = '#fff'; tc.beginPath(); tc.arc(95,110,22,0,Math.PI*2); tc.arc(160,110,22,0,Math.PI*2); tc.fill();
        tc.fillStyle = '#000'; tc.beginPath(); tc.arc(98,112,8,0,Math.PI*2); tc.arc(163,112,8,0,Math.PI*2); tc.fill();
      } else if (k === 'cameraman') {
        tc.fillStyle = '#cbd5e1'; tc.fillRect(0,0,256,140);
        tc.fillStyle = '#334155'; tc.fillRect(0,140,256,116);
        tc.fillStyle = '#0f172a'; tc.fillRect(100,70,56,130);
      } else if (k === 'shapes') {
        tc.fillStyle = '#000'; tc.fillRect(0,0,256,256);
        tc.fillStyle = '#fff'; tc.fillRect(40,40,60,60);
        tc.beginPath(); tc.arc(180,80,35,0,Math.PI*2); tc.fill();
      } else if (k === 'damaged') {
        tc.fillStyle = '#b45309'; tc.fillRect(0,0,256,256);
        tc.fillStyle = '#fff'; tc.lineWidth = 3;
        tc.beginPath(); tc.moveTo(20,20); tc.lineTo(230,230); tc.moveTo(220,20); tc.lineTo(30,220); tc.stroke();
      }
    ...
    function resetImg() {
      const d = ctxO.getImageData(0,0,256,256);
      ctxP.putImageData(d, 0, 0);
      document.getElementById('metrics').innerText = 'Ready';
      updateHist(d);
    }
    ...
    function openModal() { document.getElementById('modalReport').classList.remove('hidden'); }
    function closeModal() { document.getElementById('modalReport').classList.add('hidden'); }
  </script>
</body>
</html>
"""
